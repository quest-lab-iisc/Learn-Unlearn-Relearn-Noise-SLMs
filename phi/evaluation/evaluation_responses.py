...
def compute_similarity(csv_file1, csv_file2):
    ...
    for index in range(len(df1)):
        ...
        response = df1.loc[index, 'Answer'] #Answer_flipped for flipped cases
...
3) If the 'actual_answer' and the 'answer_model' are very similar, your response should be 'Yes'. 
...
def process_folder(folder_path, csv_file2):
    csv_files = [f for f in os.listdir(folder_path) if f.endswith('.csv') and f.startswith('processed')]
